...
expect <<EOF
  set timeout 60
  set send_slow {1 1}
  spawn pacstrap -C ./mkimage-arch-pacman.conf -c -d -G -i $ROOTFS base haveged sudo --ignore $PKGIGNORE
  expect {
...
tar --numeric-owner -C $ROOTFS -c . | docker import - codekoala/arch:latest
docker run -i -t codekoala/arch:latest echo Success.
rm -rf $ROOTFS
